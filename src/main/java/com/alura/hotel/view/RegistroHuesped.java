...
public class RegistroHuesped extends JFrame {
	...
	public RegistroHuesped(Long idReserva) {		
		...
		txtNacionalidad.setModel(new DefaultComboBoxModel(new String[] {"afgano-afgana", "alemán-", "alemana", "árabe-árabe", 
																			"argentino-argentina", "australiano-australiana", 
																			"belga-belga", "boliviano-boliviana", "brasileño-brasileña", 
																			"camboyano-camboyana", "canadiense-canadiense", "chileno-chilena", 
																			"chino-china", "colombiano-colombiana", "coreano-coreana", 
																			...
																			"haitiano-haitiana", "holandés-holandesa", "hondureño-hondureña", "indonés-indonesa", 
																			"inglés-inglesa", "iraquí-iraquí", "iraní-iraní", "irlandés-irlandesa", "israelí-israelí", 
																			"italiano-italiana", "japonés-japonesa", "jordano-jordana", "laosiano-laosiana", 
																			"letón-letona", "letonés-letonesa", "malayo-malaya", "marroquí-marroquí", 
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				//java.sql.Date birthDate1 = (java.sql.Date) txtFechaN.getDate(); 
				java.util.Date birthDateIn = txtFechaN.getDate(); //Cambio de java.util a java.sql
				java.sql.Date birthDate = new java.sql.Date(birthDateIn.getTime());
				
				String nationality = txtNacionalidad.getSelectedItem().toString();
				String phoneNumber = txtTelefono.getText();
				Long reservationId = Long.valueOf(txtNreserva.getText());
				
				if( name.trim().isEmpty() && lastName.trim().isEmpty() && nationality.trim().isEmpty() && phoneNumber.trim().isEmpty()) {
					JOptionPane.showMessageDialog(new JFrame(), "¡Por favor rellene todos los campos!" );
                    return;
				}
				guestController.saveGuesData(name, lastName, birthDate, nationality, phoneNumber, reservationId);
				dispose();
		...
		btnexit.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				int result = JOptionPane.showConfirmDialog(
	                	new JFrame(),"¡Salir del registro cancelará tu reserva!\n"
	                				+ "\n¿Deseas continuar con tu registro?" ,"¡Aviso!" ,JOptionPane.YES_NO_OPTION);
	                if(result == 1) {
	                	JOptionPane.showMessageDialog(new JFrame(), "¡Lamentamos no poder contar con tu visita! ");
	                	reservationController.deleteReservation(Math.toIntExact(idReserva));
		...
		logo.setBounds(195, 125, 104, 107);
		panel.add(logo);
		...
		imagenFondo.setIcon(new ImageIcon(RegistroHuesped.class.getResource("/images/Hotel7.jpg")));
	}
